...
namespace eagletechapi.service.implements
{
    public class UserService(AppDbContext context) : IUserService
    {
        ...
        public async Task<UsuarioOut?> AlterarSenha(PasswordUpdate passwordUpdate)
        {
            int matricula = passwordUpdate.Matricula;
            string novaSenha = passwordUpdate.SenhaNova;
            string senhaAntiga = passwordUpdate.SenhaAntiga;
            var usuario = await _context.Usuarios.FirstOrDefaultAsync(u => u.Matricula == matricula) ?? throw new Exception("Usuario não encontrado");
